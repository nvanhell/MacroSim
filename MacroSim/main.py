...
class Simulation(threading.Thread):
    def __init__(self):
        ...
        self.timer = 0  # used to keep track of the time it takes to run a period (debugging)
        self.period = 0  # Variable for the current time period of the simulation
        self.months_per_period = 1  # Number of months a worker will age per period
        ...
        # The following are variables used to store economic variables so that they can be graphed in any period
        self.x = []
        ...
        while self.simulation_on:
            ...
            for econ in economies:
                ...
                for worker in econ.workers:
                    worker.age += self.months_per_period / 12
                    if worker.age >= self.age_to_die:
                        worker.kill(worker.b)  # Passes through the old utility function parameters to the new worker
                    else:
